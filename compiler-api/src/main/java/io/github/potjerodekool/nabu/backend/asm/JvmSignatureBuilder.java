package io.github.potjerodekool.nabu.backend.asm;
...
public final class JvmSignatureBuilder {
     ...
     */
    public static void appendTypeMirror(StringBuilder sb, TypeMirror type) {
        if (type == null) {
            sb.append('L');
            ...
            case WILDCARD -> {
                ...
                if (lower != null && lower.getKind() != TypeKind.NONE) {
                    ...
                    appendTypeMirror(sb, lower);
                        ...
                        || upper.getKind() == TypeKind.NONE
                        || isJavaLangObject(upper)) {
                    sb.append('*');
                } else {
                    sb.append('+');
            ...
            case DECLARED -> {
                if (type instanceof DeclaredType declared) {
                    ...
                    if (element instanceof TypeElement te) {
                        sb.append('L');
                        sb.append(te.getQualifiedName().replace('.', '/'));
