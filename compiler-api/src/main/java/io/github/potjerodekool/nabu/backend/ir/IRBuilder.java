...
public class IRBuilder {
    ...
    public IRValue emitBinaryOp(IRInstruction.BinaryOp.Op op, IRValue left, IRValue right) {
        final var commonType = binaryNumericPromotion(left.type(), right.type());
        var operationResultType = left.type();

        if (commonType != null) {
            if (!left.type().equals(commonType)) {
                left = emitCast(left, commonType);
            }
            if (!right.type().equals(commonType)) {
    ...
    // worden ze naar een gemeenschappelijk type verbreed (double > float > long
    // > int). Het resultaat van de bewerking is dat bevorderde type.
    private static IRType binaryNumericPromotion(final IRType a,
                                                 final IRType b) {
        if (!(a instanceof IRType.Int) && !(a instanceof IRType.Float)) {
            return null;
        }
        if (!(b instanceof IRType.Int) && !(b instanceof IRType.Float)) {
            return null;
        }
        ...
        if (a instanceof IRType.Float || b instanceof IRType.Float) {
            final boolean aDouble = a instanceof IRType.Float af && af.bits() == 64;
            final boolean bDouble = b instanceof IRType.Float bf && bf.bits() == 64;
            return aDouble || bDouble ? IRType.F64 : IRType.F32;
        }

        final boolean aLong = a instanceof IRType.Int ai && ai.bits() == 64;
        final boolean bLong = b instanceof IRType.Int bi && bi.bits() == 64;
        if (aLong || bLong) {
            return IRType.I64;
        }

        // JLS 5.6.2: byte/short/char worden bevorderd naar int
        return IRType.I32;
    }
