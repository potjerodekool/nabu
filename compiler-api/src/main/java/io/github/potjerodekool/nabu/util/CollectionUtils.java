...
public final class CollectionUtils {
    ...
    public static <A, B> void forEach(final List<? extends A> first,
                                      final List<? extends B> second,
                                      final BiConsumer<A, B> consumer) {
        if (first.size() != second.size()) {
            throw new IllegalArgumentException("Inputs aren't equal in size");
        }

        final var count = first.size();

        for (int i = 0; i < count; i++) {
            final var a = first.get(i);
            final var b = second.get(i);
            consumer.accept(a, b);
        }
    }
