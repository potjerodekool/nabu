...
class Phase2ResolverTest {
    ...
    public void setup() throws URISyntaxException {
        ...
        addToClassPath(JpaPredicate.class);
        addToClassPath(Root.class);

        classElementLoader.postInit();
    }

    private void addToClassPath(final Class<?> clazz) throws URISyntaxException {
        classElementLoader.addClassPathEntry(Path.of(clazz.getProtectionDomain().getCodeSource().getLocation().toURI()));
    }
    ...
    void visitLambdaExpression() throws IOException {
        ...
        final var lambda = new LambdaExpressionTree();
        final var lambdaBody = new BlockStatement();
        lambda.body(lambdaBody);
        final var lambdaType = classElementLoader.resolveClass(JpaPredicate.class.getName()).asType();
        ...
        final var scope = new FunctionScope(globalScope, null);
