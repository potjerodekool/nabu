...
public class ClassSymbolBuilder extends AbstractSymbolBuilder<TypeElementBuilder<io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol>> implements TypeElementBuilder<io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol> {
    ...
    @Override
    public io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol build() {
        return build(false);
    }

    @Override
    public io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol buildError() {
        return build(true);
    }

    private io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol build(final boolean errorType) {
        final var typeArguments = typeParameters.stream()
                .map(TypeParameterElement::asType)
                .toList();

        final CClassType type;
        final io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol clazz;

        if (errorType) {
            final var errorSymbol = new ErrorSymbol(getSimpleName());
            errorSymbol.setKind(getKind());
            errorSymbol.setNestingKind(nestingKind);
            errorSymbol.setEnclosingElement((io.github.potjerodekool.nabu.compiler.ast.symbol.impl.Symbol) getEnclosingElement());
            errorSymbol.setEnclosedElements(getEnclosedElements());
            errorSymbol.setAnnotations(getAnnotations());
            clazz = errorSymbol;
        } else {
            clazz = new io.github.potjerodekool.nabu.compiler.ast.symbol.impl.ClassSymbol(
                    getKind(),
                    nestingKind,
                    getFlags(),
                    getSimpleName(),
                    ...
                    (io.github.potjerodekool.nabu.compiler.ast.symbol.impl.Symbol) getEnclosingElement(),
                    getEnclosedElements(),
                    getAnnotations()
            );
